...
public class MaxTest {
    ...
    public void returnsNullOnEmptyIterable() {
        assertThat(Max.create().apply(Collections.emptyList()), nullValue());
    }
    ...
    public void selectsMaxValue() {
        ...
        val subject = Max.<Integer>create(comparator);
        assertThat(subject.apply(Arrays.asList(2, 1, 3, 5, 4)), equalTo(5));
        verify(comparator, times(1)).compare(2, 1);
        verify(comparator, times(1)).compare(2, 3);
        verify(comparator, times(1)).compare(3, 5);
        verify(comparator, times(1)).compare(5, 4);
    }
}
